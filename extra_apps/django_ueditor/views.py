...
import django_ueditor.settings as USettings
...
def get_ueditor_settings(request):
    js = json.dumps(USettings.UEditorUploadSettings, ensure_ascii=False)
...
def get_files(root_path, cur_path, allow_types=[]):
    ...
    for item in items:
        ...
        if os.path.isdir(item_fullname):
            ...
            if is_allow_list:
                files.append({
                    "url": parse.urljoin(USettings.gSettings.MEDIA_URL,
...
def UploadFile(request):
    ...
    if action == "uploadscrawl":
        ...
        if file is None: return HttpResponse(json.dumps(u"{'state:'ERROR'}"))
    ...
    if max_size != 0:
        if upload_file_size > getSize(size=max_size):
            state = u"上传文件大小不允许超过%s。"
    ...
    # 返回数据
    return_info = {
        'url': parse.urljoin(USettings.gSettings.MEDIA_URL, OutputPathFormat),  # 保存后的文件名称
...
def catcher_remote_image(request):
    ...
    for remote_url in remote_urls:
        ...
        if remote_original_ext in allow_type:
            ...
            # 读取远程图片文件
            try:
                remote_image = request.urlopen(remote_url)
                # 将抓取到的文件写入文件
                try:
